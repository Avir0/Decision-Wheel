...
export default function WheelPage({ history, setHistory, setPage }) {
  ...
  const handleGenerate = () => {
    ...
    const newChoices = generateChoices(text, mode, 6)
  ...
  }, [choices, targetIndex, dilemma, mode, mood])
    ...
    <div className="min-h-screen pt-24 pb-20 px-4 relative z-10">
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          >
            What are you
            <span style={{ color: 'var(--accent)' }}> confused</span> about?
          </h1>
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
            Type your dilemma. We'll handle the rest (and the blame).
          </p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-[1fr_400px] gap-8 items-start">
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {exampleDilemmas.slice(0, 3).map((d, i) => (
                  <button
                    key={i}
                    onClick={() => setDilemma(d.replace(/[📱😴🏋️💼☮️🍕🥗💍]/g, '').trim())}
                ...
                {modeOptions.map(m => (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: mode === m.id ? '0 4px 16px var(--glow)' : 'none',
                    }}
                  >
                    {m.label}
                  </button>
                  ...
                  >
                    {moods.map(m => (
                      <button
                        ...
                        style={{
                          background: mood === m ? 'var(--accent)' : 'var(--glass)',
                          color: mood === m ? 'white' : 'var(--text-muted)',
            ...
            >
              <motion.button
                ...
                className="py-3.5 rounded-2xl text-sm font-bold glass"
                style={{ border: '1px solid var(--accent)', color: 'var(--accent)', fontFamily: 'Syne, sans-serif' }}
              >
                🤖 Generate Choices
              </motion.button>
              <motion.button
                ...
                className="py-3.5 rounded-2xl text-sm font-bold glass"
                style={{ border: '1px solid var(--glass-border)', color: 'var(--text-muted)', fontFamily: 'Syne, sans-serif', opacity: choices.length === 0 ? 0.5 : 1 }}
              >
                🔀 Shuffle
              </motion.button>
            </motion.div>
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.03, boxShadow: '0 0 60px var(--glow)' }}
              whileTap={{ scale: 0.97 }}
              onClick={handleSpin}
              disabled={spinning}
              className="w-full py-5 rounded-2xl text-base font-black text-white relative overflow-hidden"
              style={{
                background: spinning
                  ? 'var(--glass)'
                  : 'linear-gradient(135deg, var(--accent) 0%, var(--accent2) 100%)',
                boxShadow: spinning ? 'none' : '0 8px 40px var(--glow)',
          ...
          <motion.div
            ...
            className="flex flex-col items-center gap-6 lg:sticky lg:top-28"
          >
            <SpinWheel
              choices={choices}
              spinning={spinning}
            ...
            <motion.div
              ...
              className="glass rounded-2xl p-4 w-full text-center"
              style={{ border: '1px solid var(--glass-border)' }}
            >
